# =========================================
# DLNM Pipeline: Temperature–Mortality Analysis
# Author: [Your Name]
# =========================================
...
# --- 1. Load and Preprocess Temperature Data ---
load_temperature_data <- function(filepath) {
  ...
  df <- df %>%
    ...
    select(Date, Temp_Mean, Fiscal_Year)
  
  return(df)
}

# --- 2. Simulate Daily Death Counts ---
simulate_deaths <- function(df) {
  ...
  df <- df %>%
    left_join(death_table, by = "Fiscal_Year") %>%
...
build_dlnm_model <- function(df) {
  ...
  model <- glm(
    Deaths ~ cb + ns(as.numeric(Date), df = 8 * 5),
    family = quasipoisson(),
    data = df
  )
  
  return(list(cb = cb, model = model))
}

# --- 4. Predict Cumulative RR ---
generate_predictions <- function(cb, model, df) {
  pred_cum <- crosspred(
    cb, model,
    cen = 27.5,
...
save_figures <- function(pred_cum) {
  ...
  # Find nearest temperature to 32°C
...
# --- 6. Export Cumulative RR Table ---
export_rr_table <- function(pred_cum) {
  rr_table <- data.frame(
...
# --- 7. Attributable Risk Analysis ---
compute_attributable_deaths <- function(df, cb, model) {
  pred_all <- crosspred(cb, model, cen = 27.5,
  ...
  cat("Deaths attributable to non-optimal temperatures: ", round(total_attr), "\n")
  cat("Attributable Fraction: ", round(AF_total * 100, 2), "%\n")
...
run_dlnm_pipeline <- function() {
  df <- load_temperature_data("data/amd_temp.csv")
  df <- simulate_deaths(df)
  
  model_components <- build_dlnm_model(df)
  cb <- model_components$cb
  model <- model_components$model
  
  pred_cum <- generate_predictions(cb, model, df)
  
  save_figures(pred_cum)
  export_rr_table(pred_cum)
  
  df <- compute_attributable_deaths(df, cb, model)
}
